	...
	for (var y = roi.y; y < roi.y + roi.height; y++) {
		...
		var p = this.image[y*this.width + x];
                if (p === this.ignore) {
                    ...
                    if (use_rainbow) {
                        var rgb = HSVtoRGB (gray/255, 1, 1);
                        roidata[4*datai]   = rgb.r;	// red
                        roidata[4*datai+1] = rgb.g;	// green
                        roidata[4*datai+2] = rgb.b;	// blue
                    } else {
...
function FITS_handleMouse (event, thisfits)
{
    ...
    // find and handle dragging an ROI, if any
    var down = (event.buttons == undefined) ? event.which : event.buttons;
    if (down) {
	if (thisfits.drag_code == undefined)
	    thisfits.findROI (thisfits.mic);
	if (thisfits.drag_code != undefined) {
	    // set matching javascript cursor
	    thisfits.gcvs.style.cursor = thisfits.cursors[thisfits.drag_code];

	    // record click ROI center offset if just started a move
	    if (event.type == 'mousedown' && thisfits.drag_code == 'move') {
		var thisroi = thisfits.rois[thisfits.drag_roi];
		thisfits.drag_mvos = {
		    dx: Math.round((thisroi.x + thisroi.width/2) - thisfits.mic.x),
		    dy: Math.round((thisroi.y + thisroi.height/2) - thisfits.mic.y)
		};
	    }

	    // draw ROI at new location
	    thisfits.moveROI (thisfits.mic);
	}

	// record cursor click location for glass when cursor moves outside image
	if (event.type == 'mousedown')
	    thisfits.glass_mic = thisfits.mic;

    } else {
	thisfits.findROI (thisfits.mic);
	if (thisfits.drag_code == undefined)
	    thisfits.gcvs.style.cursor = "inherit";
	else
	    thisfits.gcvs.style.cursor = thisfits.cursors[thisfits.drag_code];
    }

    // call user mouse moved callback, if any
